...
<style>
    ...
    <div>
        ...
        <form action="/r09routes" method="post" enctype="multipart/form-data">@csrf
            ...
            <div class="row align-items-center mb-2" style="display:none;">
                ...
                <dd class="col-sm-9">
                    <input type="text" class="form-control" name="title" id="title" value="T9 -> ">
                </dd>
            </div>
                    ...
                    <select name="stores" class="form-control" onchange="$('#title').attr('value', this.options[this.selectedIndex].title);">
                        ...
                        @foreach($stores as $store)
                            @if(!count($store->r09routes))
                                <option value="{{ $store->id }}" title="T9 -> {{ $store->title }}">{{ $store->title }}</option>
                            @endif
                        @endforeach
                    </select>
